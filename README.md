...
* **A true reactive framework:** Active CSS has the simplest method of automatically updating drawn content based on variable changes. Why suffer with over-engineered frameworks, when you can set up easy to read CSS-style declarations for components and write your code in one-liners?
* **No virtual DOM:** Because, let's face it, a virtual DOM is an overhead if you can get around not using one at all. In Active CSS, only the content you want updating gets updated - same as any other framework that uses a virtual DOM. You can use your old plugins, and you know where you are with the DOM at any one time.
