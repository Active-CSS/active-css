...
* **A true reactive framework:** Active CSS has the simplest method of automatically updating drawn content based on variable changes. Why suffer with incredibly over-engineered frameworks, when you can set up easy to read CSS-style declarations for components and write your code in one-liners?
* **No virtual DOM:** Because we workd out how to do the same things as the other frameworks, but without a virtual DOM. Because we're awesome. Let's face it, a virtual DOM is an overhead if you don't need it. In Active CSS, only the content you want updating gets updated - same as any other framework that uses a virtual DOM. Another benefit, as well as speed, is that you can use your old plugins, and you know where you are with the DOM at any one time. Sure, there's a couple of attributes we needed to insert, but hey - small price to pay.
...
* **Simple by design:** Simple architecture of the core for optimal performance. No fush. No compromise. No BS over-engineering.
